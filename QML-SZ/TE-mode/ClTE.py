...
if exp == "Big":
    ...
    mask[abs(90 - rad2deg(t)) < glat] = False
mask_in  = hp.read_map("AliCPT.fits",field=0,dtype=bool,verbose=0)
mask = tools.degrade_mask(nside, mask_in)
fsky = np.mean(mask)
npix = sum(mask)
...
#    Wll = np.asarray( [np.sum(E * P) for E in esti.El for P in esti.Pl] ).reshape(nl,nl)
    ...
    esti.bias = np.zeros_like(xqml.estimators.biasQuadEstimator(esti.NA, esti.El))
    s2 = timeit.default_timer()
