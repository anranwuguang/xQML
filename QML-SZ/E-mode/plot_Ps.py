...
def plot_Ps(output_dir, clth_in, spec,nside,lmax,dell,fsky,index=-1):
    # # ############## Initial parameters ###############
    if fsky < 0.3:
    ...
    # # ################################################# 
    ##################  Input model  #####################
    hcla  = np.loadtxt(output_dir+"/results/hcl.txt", unpack=True)
    scla  = np.loadtxt(output_dir+"/results/scl.txt", unpack=True)
    print(hcla)
    clth = clth_in[:,lth]
    ...
    for n in np.arange(nspec):
        ithfig = n+1
        formula_errors=cal_errors(clth,n,lth,dell,fsky,index)
        if index>=0:
            clth[0]=clth[index]
            print(clth)
...
def cal_errors(clth,n,lth,dell,fsky,index):
  if index>=0:
      n = index
  if n< 3:
